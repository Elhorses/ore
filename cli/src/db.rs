...
impl RedisClient {
    ...
    pub async fn setex(&self, key: &str, value: &str, seconds: u64) -> Result<()> {
        let mut conn = self.get_connection().await?;
        let _: () = redis::cmd("SETEX").arg(key).arg(seconds).arg(value).query_async(&mut conn).await?;
        Ok(())
    }
    ...
    /// Get a hash field
    pub async fn hget<T: redis::FromRedisValue>(&self, key: &str, field: &str) -> Result<Option<T>> {
        let mut conn = self.get_connection().await?;
    ...
    pub async fn hincrby(&self, key: &str, field: &str, increment: i64) -> Result<i64> {
        let mut conn = self.get_connection().await?;
        let value: i64 = redis::cmd("HINCRBY").arg(key).arg(field).arg(increment).query_async(&mut conn).await?;
        Ok(value)
    }
    ...
    pub async fn zrange<T: redis::FromRedisValue>(&self, key: &str, start: isize, stop: isize) -> Result<Option<T>> {
        let mut conn = self.get_connection().await?;
...
mod tests {
    ...
    fn test_motherlode_calculations() {
        ...
        assert_eq!(calculate_motherlode_rounds_until_trigger(almost_full_cycle), 1);
        assert_eq!(calculate_motherlode_rounds_since_trigger(almost_full_cycle), 624);
    }
}
